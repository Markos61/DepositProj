...
def mean_target_encoding(X_train, X_val, column, y_train):
    """
    ...
    # Заполняем пропуски (если в валидации встретилась новая биграмма) средним по всему трейну
    global_mean = y_train.mean()
    # smooth_mean = (target_stats['mean'] * target_stats['count'] + global_mean * weight) /
    # (target_stats['count'] + weight)
    X_train[f'{column}_TE'] = X_train[f'{column}_TE'].fillna(global_mean)
    X_val[f'{column}_TE'] = X_val[f'{column}_TE'].fillna(global_mean)

    # Добавляем крошечный шум в трейн, чтобы модель не "зазубрила" среднее (регуляризация)
...
# Библиотека для отбора фичей
# Признак выходной/рабочий
# Label encoding попробовать
def create_new_features(df):
    ...
    df = add_feature(df, 'job', 'cat_comb', 'education')
